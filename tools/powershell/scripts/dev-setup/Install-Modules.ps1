...
.EXAMPLE
Install-Modules
Just invoke this script to install all current modules
...
.LINK
https://github.com/matAchu222/dev-setup
...
PROCESS {
    try {
        ...
        # Asks if the user wants to replace its profile with the profile
        if ($PSCmdlet.ShouldContinue("Do you want to replace your PowerShell profile with the new profile?", "This will overwrite your current profile.")) {
            ...
            Write-Warning "Profile was not replaced with new profile"
        }
